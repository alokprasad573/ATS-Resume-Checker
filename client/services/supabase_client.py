...
try:
    ...
    if local_env.exists():
        print("Loading .env from: " + str(local_env))
        load_dotenv(local_env, override=True)
    elif root_env.exists():
        print("Loading .env from: " + str(root_env))
...
def google_oauth_url() -> Dict[str, Any]:
    ...
    try:
        ...
        # Save code_verifier to session state to prevent lost state across redirect
...
def exchange_code_for_session(auth_code: str) -> Dict[str, Any]:
    ...
    try:
        ...
        code_verifier = st.session_state.get("google_code_verifier") or ''
        
        # Fallback to local storage if session state is missing
