...
from server.core.config import SUPABASE_URL,SUPABASE_KEY
...
async def save_analysis(user_id: str, filename: str, analysis_result: Dict) -> Optional[str]:
    ...
    if headers is None:
        logger.error('No Supabase creds available')
        return None

    def _json_defaults(o):
        if hasattr(o, 'model_dump'):
            return o.model_dump()
        
        return str(o)
    
    serializable_result = json.loads(json.dumps(analysis_result, default=_json_defaults))
